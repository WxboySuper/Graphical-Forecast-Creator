...
### Package rollup
Equal weight among present products: categorical / tornado / wind / hail. Absent → **Not evaluated**.

### Data quality
- **Blocked:** malformed geometry, impossible/missing dates, unreadable package, failed required SPC fetch with no usable rows → no run.
...
### Categorical → probability map (version in constants)
TSTM 0.05, MRGL 0.10, SLGT 0.15, ENH 0.30, MDT 0.40, HIGH 0.45; hazard probs 2/5/10/15/30/45/60 as usual. Significant contours feed **severity**, not the main Brier probability field.
