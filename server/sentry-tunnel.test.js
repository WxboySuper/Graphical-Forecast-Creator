...
afterEach(() => { global.fetch = originalFetch; delete process.env.SENTRY_BROWSER_DSN; delete process.env.SENTRY_DSN; });
...
test('returns 504 when upstream aborts at the timeout', async () => { const route = setupRoute(); const request = createRequest(); const response = createResponse(); global.fetch = (_url, options) => new Promise((_resolve, reject) => options.signal.addEventListener('abort', () => reject(Object.assign(new Error('aborted'), { name: 'AbortError' })))); await route(request, response); assert.equal(response.statusCode, 504); });
